<template>
  <div class="chart-container">
    <div ref="chartContainer" id="tech_charts3"></div>
  </div>
</template>
...
const fetchChartData = async (prov) => {
  try {
    console.log("tech_chart3请求的省份：", prov);
    const response = await axios.get(`http://localhost:8080/tech/chart3/${prov}`);
    console.log("tech_chart3返回的数据", response.data);
    if (
      ...
      console.error("tech_chart3返回数据格式错误", response.data);
    }
  } catch (error) {
    console.error("tech_chart3数据请求失败", error);
  }
};
