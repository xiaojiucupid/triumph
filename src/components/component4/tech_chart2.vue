<template>
  <div class="chart-container">
    <div ref="chartContainer" id="tech_charts2"></div>
  </div>
</template>
...
const fetchChartData = async (prov) => {
  try {
    console.log("tech_chart2请求的省份：", prov);
    const response = await axios.get(`http://localhost:8080/tech/chart2/${prov}`);
    console.log("tech_chart2返回的数据", response.data);
    if (
      ...
      console.error("tech_chart2返回数据格式错误", response.data);
    }
  } catch (error) {
    console.error("tech_chart2数据请求失败", error);
  }
};
